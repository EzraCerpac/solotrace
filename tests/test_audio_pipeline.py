...
from solotrace.audio import create_preview_stems, transcribe_pyin


def test_pyin_transcription_keeps_last_onset_segment(tmp_path) -> None:
    sample_rate = 22_050
    ...
    for index, frequency in enumerate((220.0, 261.63, 329.63)):
        ...
        audio[start : start + length] = (
            np.sin(2 * np.pi * frequency * time) * envelope * 0.5
        )
...
def test_pyin_honors_drop_d_range_and_repeated_attacks(tmp_path) -> None:
    ...
    for start_s in (0.2, 0.85):
        ...
        envelope = np.sin(np.linspace(0, np.pi, length)) ** 0.35
        audio[start : start + length] += (
            np.sin(2 * np.pi * 73.416 * time) * envelope * 0.5
        )
...
def test_preview_stems_stream_full_song_and_limit_lead_to_passage(tmp_path) -> None:
    sample_rate = 22_050
    seconds = 4
