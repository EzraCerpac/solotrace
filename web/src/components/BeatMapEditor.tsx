...
export function BeatMapEditor({
  ...
  // Dense transcriptions can contain thousands of onset guides. One SVG path
  // keeps the visual identical without reconciling one React node per note on
  // every playhead update.
  const noteOnsetsPath = useMemo(
    () => noteOnsets
      .map((seconds) => {
        const x = clamp(seconds / Math.max(duration, 0.001), 0, 1) * waveformWidth
        return `M ${x.toFixed(2)} ${waveformHeight - 18} V ${waveformHeight}`
      })
      .join(' '),
    [duration, noteOnsets, waveformWidth],
  )
  ...
  const tapTempo = () => {
    ...
    const compoundPulse =
      valueRef.current.time_signature[0] === 6 &&
      valueRef.current.time_signature[1] === 8
    const tempo = clamp(
      Math.round((compoundPulse ? 90_000 : 60_000) / average),
      20,
      400,
    )
      ...
      <div className="beat-map-editor__control-grid">
        ...
        <fieldset className="beat-map-editor__control-card">
          ...
          <button
            ...
            className="button secondary"
            disabled={saving}
            onClick={alignDownbeatToPlayhead}
          >
            Align downbeat to playhead
          </button>
          ...
          <p className="beat-map-editor__hint">
            This shifts the repeating bar grid; it does not renumber the selected
            point as Bar 1. Downbeats sound accented in click preview.
          </p>
        </fieldset>
      </div>
